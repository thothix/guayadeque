    ...
    wxString GetSongsSortSQL(const wxArrayInt *aorder, const wxArrayInt *aorderdesc, const bool force_fallback = false)
    {
        ...
        for (size_t i = 0; i < aorder->GetCount(); i++)
        {
            switch (aorder->Item(i))
            {
                ...
                case guTRACKS_ORDER_COMPOSER:
                    query += GetOrderOnce(SAR) + GetOrderOnce(SAL) + GetOrderOnce(SD) + GetOrderOnce(SAI) + GetOrderOnce(SN);
                    break;

                case guTRACKS_ORDER_ARTIST:
                case guTRACKS_ORDER_ALBUMARTIST:
                    query += GetOrderOnce(SY) + GetOrderOnce(SAL) + GetOrderOnce(SD) + GetOrderOnce(SAI) + GetOrderOnce(SN);
                    break;

                case guTRACKS_ORDER_ALBUM:
                    query += GetOrderOnce(SD) + GetOrderOnce(SAI) + GetOrderOnce(SN);
                    break;

                case guTRACKS_ORDER_YEAR:
                    query += GetOrderOnce(SAL) + GetOrderOnce(SD) + GetOrderOnce(SAI) + GetOrderOnce(SN);
                    break;
    ...
    */
    ...
    void guDbLibrary::CleanFiles(const wxArrayString &Files)
    {
        ...
        for (int Index = 0; Index < Count; Index++)
        {
            ...
            {
                int TrackId = FindTrackFile(Files[Index], nullptr);
                if (TrackId)
                    Tracks.Add(TrackId);
            } else if (guIsValidImageFile(Files[Index].Lower()))
            {
                query = wxString::Format(
                    wxT("SELECT cover_id FROM covers WHERE cover_path = '%s'"), escape_query_str(Files[Index]).c_str());
                dbRes = ExecuteQuery(query);
                if (dbRes.NextRow())
    ...
    void guDbLibrary::CleanItems(const wxArrayInt &tracks, const wxArrayInt &covers)
    {
        ...
        //  ExecuteUpdate( query );
        query = wxT(
            "DELETE FROM plsets WHERE plset_type = 0 AND plset_songid NOT IN ( SELECT DISTINCT song_id FROM songs );");
        ExecuteUpdate(query);
    ...
    // -------------------------------------------------------------------------------- //
    bool guDbLibrary::CheckDbVersion()
    {
        wxArrayString query;
        unsigned long dbVer;
        ...
        switch (dbVer)
        {
            ...
            case 0:
            {
                ...
                query.Add(wxT(
                    "CREATE INDEX IF NOT EXISTS song_albumid on songs( song_albumid,song_artist,song_year DESC, song_album, song_disk )"));
                ...
                query.Add(wxT(
                    "CREATE INDEX IF NOT EXISTS song_albumartistid on songs( song_albumartistid, song_albumartist )"));
                ...
                query.Add(wxT(
                    "CREATE INDEX IF NOT EXISTS song_genre ON songs( song_genre,song_composer,song_artist,song_album,song_disk,song_albumid,song_number )"));
                query.Add(wxT(
                    "CREATE INDEX IF NOT EXISTS song_genre_desc ON songs( song_genre DESC,song_composer,song_artist,song_album,song_disk,song_albumid,song_number )"));
                query.Add(wxT(
                    "CREATE INDEX IF NOT EXISTS song_composer ON songs( song_composer,song_artist,song_album,song_disk,song_albumid,song_number )"));
                query.Add(wxT(
                    "CREATE INDEX IF NOT EXISTS song_composer_desc ON songs( song_composer DESC,song_artist,song_album,song_disk,song_albumid,song_number )"));
                query.Add(wxT(
                    "CREATE INDEX IF NOT EXISTS song_artist ON songs( song_artist,song_album,song_disk,song_albumid,song_number )"));
                query.Add(wxT(
                    "CREATE INDEX IF NOT EXISTS song_artist_desc ON songs( song_artist DESC,song_album,song_disk,song_albumid,song_number )"));
                query.Add(wxT(
                    "CREATE INDEX IF NOT EXISTS song_albumartist ON songs( song_albumartist,song_album,song_disk,song_albumid,song_number )"));
                query.Add(wxT(
                    "CREATE INDEX IF NOT EXISTS song_albumartist_desc ON songs( song_albumartist DESC,song_album,song_disk,song_albumid,song_number )"));
                query.Add(wxT(
                    "CREATE INDEX IF NOT EXISTS song_album ON songs( song_album,song_disk,song_albumid,song_number )"));
                query.Add(wxT(
                    "CREATE INDEX IF NOT EXISTS song_album_desc ON songs( song_album DESC,song_disk,song_albumid,song_number )"));
                query.Add(wxT("CREATE INDEX IF NOT EXISTS song_disk ON songs( song_disk,song_albumid,song_number )"));
                query.Add(wxT(
                    "CREATE INDEX IF NOT EXISTS song_disk_desc ON songs( song_disk DESC,song_albumid,song_number )"));
                query.Add(wxT(
                    "CREATE INDEX IF NOT EXISTS song_year ON songs( song_year,song_album,song_disk,song_albumid,song_number )"));
                query.Add(wxT(
                    "CREATE INDEX IF NOT EXISTS song_year_desc ON songs( song_year DESC,song_album,song_disk,song_albumid,song_number )"));
                query.Add(wxT(
                    "CREATE INDEX IF NOT EXISTS song_addedtime ON songs( song_addedtime,song_album,song_disk,song_albumid,song_number )"));
                query.Add(wxT(
                    "CREATE INDEX IF NOT EXISTS song_addedtime_desc ON songs( song_addedtime DESC,song_album,song_disk,song_albumid,song_number )"));

                query.Add(wxT(
                    "CREATE TABLE IF NOT EXISTS tags( tag_id INTEGER PRIMARY KEY AUTOINCREMENT, tag_name VARCHAR COLLATE NOCASE );"));
                ...
                query.Add(wxT(
                    "CREATE TABLE IF NOT EXISTS settags( settag_tagid INTEGER, settag_artistid INTEGER, settag_albumid INTEGER, settag_songid INTEGER );"));
                ...
                query.Add(wxT(
                    "CREATE TABLE IF NOT EXISTS playlists( playlist_id INTEGER PRIMARY KEY AUTOINCREMENT, playlist_name VARCHAR COLLATE NOCASE, "
                ...
                query.Add(wxT(
                    "CREATE TABLE IF NOT EXISTS plsets(plset_id INTEGER PRIMARY KEY AUTOINCREMENT, plset_plid INTEGER, plset_songid INTEGER, "
            ...
            case 6:
            {
                if (dbVer > 4)
                {
                    ...
                    query.Add(wxT(
                        "CREATE TABLE IF NOT EXISTS radiostations( radiostation_id INTEGER PRIMARY KEY AUTOINCREMENT, "
                    ...
                    query.Add(wxT(
                        "CREATE INDEX IF NOT EXISTS 'radiostation_genreid' on radiostations (radiostation_source,radiostation_genreid ASC);"));
                    query.Add(wxT(
                        "CREATE INDEX IF NOT EXISTS 'radiostation_lc' on radiostations (radiostation_lc ASC);"));
                    query.Add(wxT(
                        "CREATE INDEX IF NOT EXISTS 'radiostation_type' on radiostations (radiostation_type ASC);"));
                    query.Add(wxT(
                        "CREATE INDEX IF NOT EXISTS 'radiostation_ct' on radiostations (radiostation_ct ASC);"));
                }
            }
            ...
            case 7:
            {
                ...
                query.Add(wxT(
                    "CREATE TABLE IF NOT EXISTS covers(cover_id INTEGER PRIMARY KEY AUTOINCREMENT, cover_path VARCHAR(1024), cover_thumb BLOB, cover_midsize BLOB, cover_hash VARCHAR(32));"));
                //query.Add( wxT( "CREATE UNIQUE INDEX IF NOT EXISTS 'cover_id' on covers (cover_id ASC);" ) );
            }
            ...
            case 10:
            {
                ...
                {
                    query.Add(wxT(
                        "UPDATE playlists SET playlist_sorttype = playlist_sorttype + 1 WHERE playlist_sorttype > 4;"));
            ...
            case 15:
            {
                if (dbVer > 4)
                {
                    ...
                    query.Add(wxT(
                        "INSERT INTO songs1( song_id, song_name, song_albumid, song_artistid, song_genreid, song_filename, "
                    ...
                    query.Add(wxT(
                        "CREATE INDEX IF NOT EXISTS song_albumid on songs( song_albumid,song_artist,song_year DESC, song_album, song_disk )"));
                    query.Add(wxT(
                        "CREATE INDEX IF NOT EXISTS song_composerid on songs( song_composerid, song_composer )"));
                    query.Add(wxT("CREATE INDEX IF NOT EXISTS song_artistid on songs( song_artistid, song_artist )"));
                    query.Add(wxT(
                        "CREATE INDEX IF NOT EXISTS song_albumartistid on songs( song_albumartistid, song_albumartist )"));
                    query.Add(wxT("CREATE INDEX IF NOT EXISTS song_genreid on songs( song_genreid, song_genre )"));
                    ...
                    query.Add(wxT(
                        "CREATE INDEX IF NOT EXISTS song_album_desc ON songs( song_album DESC,song_disk,song_albumid,song_number )"));
                    query.Add(
                        wxT("CREATE INDEX IF NOT EXISTS song_disk ON songs( song_disk,song_albumid,song_number )"));
                    query.Add(wxT(
                        "CREATE INDEX IF NOT EXISTS song_disk_desc ON songs( song_disk DESC,song_albumid,song_number )"));
                    query.Add(wxT(
                        "CREATE INDEX IF NOT EXISTS song_year ON songs( song_year,song_album,song_disk,song_albumid,song_number )"));
            ...
            case 16: // Added Album Artist to dynamic playlist
            {
                ...
                {
                    query.Add(wxT(
                        "UPDATE playlists SET playlist_sorttype = playlist_sorttype + 1 WHERE playlist_sorttype > 1;"));
            ...
            case 18:
            {
                if (dbVer > 4)
                {
                    ...
                    query.Add(wxT(
                        "CREATE INDEX IF NOT EXISTS song_path_filename ON songs( song_path, song_filename )"));
                    query.Add(wxT("CREATE UNIQUE INDEX IF NOT EXISTS 'tag_name' on tags (tag_name ASC);"));
                }
            }
            ...
            if (TmpImg.IsOk())
            {
                ...
                if (MidImg.IsOk())
                {
                    ...
                    wxSQLite3Statement stmt = m_Db->PrepareStatement(wxString::Format(wxT(
                        "INSERT INTO covers( cover_id, cover_path, cover_thumb, cover_midsize, cover_hash ) "
    ...
    int guDbLibrary::AddCoverImage(const wxImage &image)
    {
        ...
        if (MidImg.IsOk())
        {
            ...
            if (TinyImg.IsOk())
            {
                wxSQLite3Statement stmt = m_Db->PrepareStatement(wxT(
                    "INSERT INTO covers( cover_id, cover_path, cover_thumb, cover_midsize, cover_hash ) "
                    "VALUES(NULL, '', ?, ?, '' );"));
                try
                {
            ...
            if (TmpImg.IsOk())
            {
                ...
                if (MidImg.IsOk())
                {
                    ...
                    wxSQLite3Statement stmt = m_Db->PrepareStatement(wxString::Format(
                        wxT(
                            "UPDATE covers SET cover_thumb = ?, cover_midsize = ?, cover_hash = '%s' WHERE cover_id = %u;"),
                        coverhash.c_str(), coverid));
    ...
    int guDbLibrary::SetAlbumCover(const int AlbumId, const wxImage &image)
    {
        ...
        if (image.IsOk())
        {
            ...
            query = wxString::Format(
                wxT("UPDATE songs SET song_coverid = %i WHERE song_albumid = %i;"), CoverId, AlbumId);
    ...
    int guDbLibrary::PathExists(const wxString &path)
    {
        ...
        wxSQLite3ResultSet dbRes;
        int RetVal = wxNOT_FOUND;
        wxString PathValue = GetPathAddTrailSep(path);
    ...
    // -------------------------------------------------------------------------------- //
    int guDbLibrary::GetSongId(wxString &filename, const int pathid, const time_t filedate, const int start,
                               bool *created)
    {
        ...
        {
            query = query.Format(wxT(
                "INSERT INTO songs( song_id, song_pathid, song_rating, song_playcount, song_addedtime ) "
    ...
    // -------------------------------------------------------------------------------- //
    int guDbLibrary::GetSongId(wxString &FileName, wxString &FilePath, const time_t filedate, const int start,
                               bool *created)
    {
    ...
    int guDbLibrary::ReadFileTags(const wxString &filename, const bool allowrating)
    {
                ...
                if (wxFileExists(CueItem.m_TrackPath))
                {
                    ...
                    if (TagInfo)
                    {
                        if (TagInfo->Read())
                        {
                            ...
                            CurTrack.m_AlbumId = GetAlbumId(CurTrack.m_AlbumName, CurTrack.m_ArtistId,
                                                            CurTrack.m_PathId, CurTrack.m_Path);
                            ...
                            if (TagInfo->m_Length)
                                CurTrack.m_FileSize =
                                        (guGetFileSize(CueItem.m_TrackPath) * CueItem.m_Length) / TagInfo->m_Length;
                            else
                            ...
                            CurTrack.m_SongId = GetSongId(CurTrack.m_FileName, CurTrack.m_PathId,
                                                          GetFileLastChangeTime(filename), CurTrack.m_Offset,
                                                          &IsNewTrack);

                            UpdateSong(CurTrack, IsNewTrack || allowrating);
                        } else
                            guLogError(
                                wxT("guDbLibrary.ReadFileTags.cue: Cannot read tags from '%s'"), filename.c_str());
        ...
        } else
        {
            ...
            CurTrack.m_Path = GetPathAddTrailSep(wxPathOnly(filename));
            ...
            if (TagInfo->Read())
            {
                ...
                guLogError(wxT("guDbLibrary.ReadFileTags.tag: Cannot read tags from '%s'"), filename.c_str());
            ...
            if (CurTrack.m_AlbumName.IsEmpty() && !CurTrack.m_Path.IsEmpty())
                CurTrack.m_AlbumName = CurTrack.m_Path.BeforeLast(wxT('/')).AfterLast(wxT('/'));
            CurTrack.m_AlbumId = GetAlbumId(CurTrack.m_AlbumName, CurTrack.m_ArtistId, CurTrack.m_PathId,
                                            CurTrack.m_Path);
    ...
    // -------------------------------------------------------------------------------- //
    void guDbLibrary::UpdateImageFile(const char *filename, const char *saveto, const wxBitmapType type,
                                      const int maxsize)
    {
        wxString query;
        wxSQLite3ResultSet dbRes;
    ...
    void guDbLibrary::GetArtists(guListItems *Artists, const bool FullList)
    {
        ...
        if (!FullList)
        {
            ...
            if (DirectoryHasPath() || m_TeFilters.Count() || m_LaFilters.Count() || m_GeFilters.Count() || m_AAFilters.
                Count() || m_CoFilters.Count())
    ...
    void guDbLibrary::GetAlbumArtists(guListItems *Items, const bool FullList)
    {
        ...
        if (!FullList)
        {
            ...
            if (DirectoryHasPath() || m_TeFilters.Count() || m_LaFilters.Count() || m_GeFilters.Count() || m_CoFilters.
                Count())
    ...
    bool guDbLibrary::GetAlbumDetails(const int albumid, int *year, int *trackcount)
    {
        ...
        bool RetVal = false;
        query = wxString::Format(
            wxT("SELECT MAX(song_year), COUNT(song_id) FROM songs WHERE song_albumid = %i;"), albumid);
                               ...
                               const wxArrayString &textfilters, const int start, const int count, const int order)
    {
        ...
        query = wxT(
            "SELECT song_albumid, song_album, song_artistid, song_albumartist, song_artist, song_coverid FROM songs ");
        if (filter)
        {
    ...
    int guDbLibrary::GetStaticPlayList(const wxString &path)
    {
        ...
        query = wxString::Format(
            wxT("SELECT playlist_id FROM playlists WHERE playlist_path = '%s' AND playlist_type = 0 LIMIT 1"),
    ...
    int guDbLibrary::CreateStaticPlayList(const wxString &path, const wxArrayInt &songs)
    {
        ...
        query = wxString::Format(
            wxT(
                "INSERT INTO playlists( playlist_id, playlist_name, playlist_type, playlist_path ) VALUES(NULL, '%s', 0, '%s' );"),
        ...
        if (PlayListId)
        {
            ...
            for (int index = 0; index < count; index++)
            {
                ...
                query = wxString::Format(wxT(
                    "INSERT INTO plsets(plset_id, plset_plid, plset_songid, plset_type, plset_option, plset_text, plset_option2 ) "
    ...
    void guDbLibrary::UpdateStaticPlayListFile(const int plid)
    {
        ...
        if (!PlaylistPath.IsEmpty())
        {
            ...
            if (FileName.Normalize(
                wxPATH_NORM_DOTS | wxPATH_NORM_TILDE | wxPATH_NORM_CASE | wxPATH_NORM_ABSOLUTE | wxPATH_NORM_LONG |
                wxPATH_NORM_SHORTCUT | wxPATH_NORM_ENV_VARS))
            {
                guTrackArray Tracks;
    ...
    int guDbLibrary::CreateDynamicPlayList(const wxString &name, const guDynPlayList *playlist)
    {
        ...
        if (PlayListId)
        {
            ...
            for (int index = 0; index < count; index++)
            {
                ...
                query = wxString::Format(wxT(
                    "INSERT INTO plsets(plset_id, plset_plid, plset_songid, plset_type, plset_option, plset_text, plset_number, plset_option2 ) "
    ...
    // -------------------------------------------------------------------------------- //
    int guDbLibrary::GetPlayListsCount()
    {
        int RetVal = 0;
    ...
    const wxString DynPlayListToSQLQuery(guDynPlayList *playlist)
    {
        ...
        for (int index = 0; index < count; index++)
        {
            ...
            switch (playlist->m_Filters[index].m_Type)
            {
                ...
                case guDYNAMIC_FILTER_TYPE_LABEL: // LABEL
                    ...
                    {
                        query += wxT(
                            "( song_id NOT IN ( SELECT DISTINCT settag_songid FROM settags WHERE settag_songid > 0 ) ) ");
                    } else
                    {
                        ...
                        query += wxT("((song_artistid ") + Option + wxT(
                            "(SELECT settag_artistid FROM settags WHERE settag_tagid IN (");
                        query += TagNameFilter + wxT(") and settag_artistid > 0 )) ") + Condition;
                        query += wxT("(song_albumid ") + Option + wxT(
                            "(SELECT settag_albumid FROM settags WHERE settag_tagid IN (");
                        query += TagNameFilter + wxT(") and settag_albumid > 0 )) ") + Condition;
                        query += wxT("(song_id ") + Option + wxT(
                            "(SELECT settag_songid FROM settags WHERE settag_tagid IN (");
        ...
        if (playlist->m_Sorted)
        {
            ...
            {
                case guDYNAMIC_FILTER_ORDER_TITLE: sort += wxT("song_name");
                    break;
                case guDYNAMIC_FILTER_ORDER_ARTIST: sort += wxT("song_artist");
                    break;
                case guDYNAMIC_FILTER_ORDER_ALBUMARTIST: sort += wxT("song_albumartist");
                    break;
                case guDYNAMIC_FILTER_ORDER_ALBUM: sort += wxT("song_album");
                    break;
                case guDYNAMIC_FILTER_ORDER_GENRE: sort += wxT("song_genre");
                    break;
                case guDYNAMIC_FILTER_ORDER_COMPOSER: sort += wxT("song_composer");
                    break;
                case guDYNAMIC_FILTER_ORDER_YEAR: sort += wxT("song_year");
                    break;
                case guDYNAMIC_FILTER_ORDER_RATING: sort += wxT("song_rating");
                    break;
                case guDYNAMIC_FILTER_ORDER_LENGTH: sort += wxT("song_length");
                    break;
                case guDYNAMIC_FILTER_ORDER_PLAYCOUNT: sort += wxT("song_playcount");
                    break;
                case guDYNAMIC_FILTER_ORDER_LASTPLAY: sort += wxT("song_lastplay");
                    break;
                case guDYNAMIC_FILTER_ORDER_ADDEDDATE: sort += wxT("song_addedtime");
                    break;
                case guDYNAMIC_FILTER_ORDER_RANDOM: sort += wxT("RANDOM()");
                    break;
            }
            ...
            switch (playlist->m_SortType)
            {
                case guDYNAMIC_FILTER_ORDER_ARTIST: sort += wxT(", song_album, song_disk, song_number");
                    break;
                case guDYNAMIC_FILTER_ORDER_ALBUMARTIST: sort += wxT(", song_album, song_disk, song_number");
                    break;
                case guDYNAMIC_FILTER_ORDER_ALBUM: sort += wxT(", song_disk, song_number");
                    break;
                case guDYNAMIC_FILTER_ORDER_GENRE: sort += wxT(", song_artist, song_album, song_disk, song_number");
                    break;
                case guDYNAMIC_FILTER_ORDER_COMPOSER: sort += wxT(", song_album, song_disk, song_number");
                    break;
                case guDYNAMIC_FILTER_ORDER_YEAR: sort += wxT(", song_album, song_disk, song_number");
                    break;
                case guDYNAMIC_FILTER_ORDER_RATING: sort += wxT(", song_album, song_disk, song_number");
                    break;
                case guDYNAMIC_FILTER_ORDER_ADDEDDATE: sort += wxT(", song_album, song_disk, song_number");
                    break;
            }
        }
    ...
    int guDbLibrary::GetPlayListSongs(const int plid, guTrackArray *tracks)
    {
        ...
        if (PlType != wxNOT_FOUND)
            return GetPlayListSongs(plid, PlType, tracks, nullptr, nullptr);
                                   ...
                                   const int filterallow, const int filterdeny)
    {
        ...
        if (!Filters.IsEmpty())
            ...
            query += wxT(" WHERE");
        query += wxString::Format(
            wxT(" song_artist = '%s' AND song_name = '%s' LIMIT 1;"), ArtistName.c_str(), TrackName.c_str());
    ...
    int guDbLibrary::FindTrackFile(const wxString &filename, guTrack *track)
    {
        ...
        wxSQLite3ResultSet dbRes;
        int RetVal = 0;
        wxString Path = GetPathAddTrailSep(wxPathOnly(filename));
    ...
    // -------------------------------------------------------------------------------- //
    void guDbLibrary::GetSongsCounters(const guTreeViewFilterArray &filters, const wxArrayString &textfilters,
    ...
    // -------------------------------------------------------------------------------- //
    int guDbLibrary::GetSongs(const guTreeViewFilterArray &filters, guTrackArray *Songs,
                              const wxArrayString &textfilters, const wxArrayInt order, const wxArrayInt orderdesc)
    {
        wxString query;
        wxSQLite3ResultSet dbRes;
        guTrack *Song;
    ...
    guArrayListItems guDbLibrary::GetArtistsLabels(const wxArrayInt &Artists)
    {
        ...
        guArrayListItem *CurItem = nullptr;
        int Id;
    ...
    int guDbLibrary::GetEmptyCovers(guCoverInfos &coverinfos)
    {
        ...
        while (dbRes.NextRow())
        {
            coverinfos.Add(new guCoverInfo(dbRes.GetInt(0), dbRes.GetString(1), dbRes.GetString(2),
                                           dbRes.GetString(3)));
        }
        dbRes.Finalize();
    ...
    {
        int TrackId = FindTrackFile(oldname, nullptr);
        //guLogMessage( wxT( "The track %s was found with id %i" ), oldname.c_str(), TrackId );
        if (TrackId)
        {
            wxString NewPath = GetPathAddTrailSep(wxPathOnly(newname));
            int PathId = GetPathId(NewPath);

            wxString query = wxString::Format(
                wxT("UPDATE songs SET song_filename = '%s', song_pathid = %i, song_path = '%s' WHERE song_id = %u;"),
